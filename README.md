# FASRC Development Instructions

*The following instructions assume development in a GitHub Codepsace, though should work with a local Docker installation*

### Starting Drupal

In a terminal, build/start the container for the (default) `drupal` service defined in the Compose file ([compose.yaml](compose.yaml)):

```
docker compose up
```

After the container has built & started, click the "Open in Browser" button in the VS Code "Your application running on port 80 is available" dialog box.

Access the Finder app at: `http://localhost/finder` (if using local Docker or GitHub Codespace via VS Code remote tunnel extension) or `https://<codespace>.github.dev/finder` (if using a GitHub Codespace via web browser).

### Logging in to Drupal

In the browser navbar, replace the URL path after the domain name in your web browser with `/user` (e.g., `http://localhost/user` or `https://<codespace-name>.github.dev/user`).

Log in with user `admin`, password `admin`.

### Updating

See [docs/Finder_module.pdf](docs/Finder_module.pdf) for instructions on how to update the storage services displayed by this site via the Drupal web interface.

*Some changes will require clearing the Drupal cache before they are reflected in the web interface; restarting the container automatically clears the cache in the entrypoint.sh script*

### Testing

To test the static site generated by the tome plugin:

```
docker compose run static
python3 -mhttp.server -d html 8080
```

Point your web browser at (e.g.) `https://localhost:8080/finder`.

If modifying any file in the repository other than the README or ./db/drupal.seqlite, rebuild the container before testing:
...
### Deploying into production (GitHub Pages)
